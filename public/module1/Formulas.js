...
function calculateBOM() {
  ...
  let isCustomCLNS =
    document.getElementById("CustomCLNSCheck") &&
    document.getElementById("CustomCLNSCheck").checked;
  ...
  const isTPVisible = !document
    .getElementById("TopPlateSection")
  ...
  const match =
    typeof panelData !== "undefined"
      ? panelData.find((p) => p.name === PModInput)
      : null;
  ...
  if (typeof walkwayList !== "undefined") {
    walkwayList.forEach((w) => {
      if (w.type === "horizontal") {
        sumHoriSz += w.size;
        countHori++;
        horiSizes.push(w.size);
      } else {
        sumVertiSz += w.size;
        countVerti++;
        vertiSizes.push(w.size);
      }
    });
  }
  ...
  if (Orient === "Portrait") {
    ...
    if (RaftCalMode === "RafExt") {
      ...
      calcCutSzPVhole = PVholeStart;
      calcCutSz = RaftCalVal;
    } else if (RaftCalMode === "RaftCutPurEdge") {
      FinalRafLen = RawSlope - PVholeStart * 2;
      calcCutSzPVhole = PVholeStart;
      calcCutSz = 0;
    } else {
      FinalRafLen = RawSlope;
      calcCutSzPVhole = 0;
      calcCutSz = 0;
    }
    FinalPurLen = RawWidth;
    DiaCalc = ColH / CosTilt;
  } else {
    let RawSlope =
      PW * PVQtyNS +
      PVGapNS * (PVQtyNS - 1) +
    ...
    if (RaftCalMode === "RafExt") {
      ...
      FinalRafLen = RawWidth;
      calcCutSzPVhole = 0;
      calcCutSz = 0;
    }
    FinalPurLen = RawSlope;
    DiaCalc = ColW / CosTilt;
  }
  ...
  let PtoPNS =
    ColQtyNS > 1
      ? Math.sqrt(Math.pow(LastPoleCalc1, 2) - Math.pow(GapFact, 2)) /
        (ColQtyNS - 1)
      : 0;
  ...
  let resHTML = `
        ...
        <span class="result-value">${formatOutput(FinalRafLen)}</span>
        </div>
        ...
        <div class="result-box" style="background:#333; border-radius: 10px; padding-left:10px; padding-right:10px;">
        <span class="result-label" style="color:#fff;">Total System Capacity:</span>
        <span class="result-value" style="color:#fff;">${SystemCapacityKW.toFixed(2)} KW</span>
        </div>
    `;
  ...
  if (ColQtyNS > 1) {
    ...
    for (let i = 1; i < ColQtyNS; i++)
      poleData.push({ id: i + 1, val: FstPoleH + step * i });
  }
  ...
  poleData.forEach((p) => {
    ...
    <div class="result-box">
      <span class="result-label">
        Vertical Support (Column/Pole)-${p.id}
      </span>
  ...
  document.getElementById("poleTable").innerHTML = poleHTML;
  ...
  window.lastBOMResults = {
    ...
    Orient,
    PurProfileText:
      document.getElementById("PurProfile").options[
        document.getElementById("PurProfile").selectedIndex
      ]?.text,
    RafProfileText:
      document.getElementById("RafProfile").options[
        document.getElementById("RafProfile").selectedIndex
      ]?.text,
    ColProfileText:
      document.getElementById("ColProfile").options[
        document.getElementById("ColProfile").selectedIndex
      ]?.text,
    FinalRafLen,
    FinalPurLen,
    SFH,
  ...
  //  AUTO REFRESH DRAWINGS (LIVE UPDATE)
  if (window.currentView) {
    const activeBtn = document.querySelector(".nav-btn.active");
    setView(window.currentView, activeBtn);
  } else {
